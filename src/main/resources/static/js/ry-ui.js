    ...
    $.extend({
    	...
    	table: {
    		...
    		exportExcel: function(formId) {
    			$.modal.confirm("确定导出该搜索条件的所有" + $.table._option.modalName + "吗？", function() {
					$('#ids').val("");
    				var rows = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
					if(rows.length==0){
						$('#ids').val("");
            			...
            			$.ajax({
							...
							var timer = window.setInterval(function () {
								$.ajax({
									type: 'post',
									dataType: 'json',
									url: ctx + "system/user/flushProgress",
									success: function (data) {
										if(data.percentText === undefined ){
										}else{
											$(".progress").css('width',data.percentText)
											$(".progress").html(data.percentText);
										}
									},
									error: function (data) {
										window.clearInterval(timer);
			...
			//车辆导入
			vehicleImportExcel:function(formId){
				var currentId = $.common.isEmpty(formId) ? 'importTpl' : formId;
				layer.open({
					type: 1,
					area: ['400px', '230px'],
					...
					btn1: function(index, layero){
						...
						if(fileSize>10485760){//上传文件不得大于10MB
							$.modal.msgWarning("请上传不得大于10MB的文件");
							return false;
						}
						var index = layer.load(2, {shade: false});
						$.modal.disable();
						...
						window.setTimeout(function() {
							...
							var timer = window.setInterval(function () {
								$.ajax({
									type: 'post',
									dataType: 'json',
									url: ctx + "ebike/vehicleManagement/flushProgress",
									success: function (data) {
										if(data.percentText === undefined ){
										}else{
											$(".progress").css('width',data.percentText)
			...
			//日期格式
			changeDatesFormat: function(cellval) {
				if (cellval != null) {
					var date = new Date(parseInt((cellval + "").replace("/Date(", "").replace(")/", ""), 10));
					var month = date.getMonth() + 1 < 10 ? "0" + (date.getMonth() + 1) : date.getMonth() + 1;
					var currentDate = date.getDate() < 10 ? "0" + date.getDate() : date.getDate();
    	...
    	modal: {
			...
			preRegisterDeal: function deal(title, url, width, height, callback) {
				layer.open({
					type: 2,
					area: ['90%', '95%'],
					fix: false,
					...
					success: function (layer) {
						var btns = layer.find('.layui-layer-btn');
						btns.find('.layui-layer-btn0').attr({
							style: 'background-color :#1E9FFF'
						});
						// btns.find('.layui-layer-btn1').attr({
						// 	style: 'background-color :#ef104e'
						// });
					},
					// yes: function (index, layero) {
					//     var iframeWin = layero.find('iframe')[0];
					//     iframeWin.contentWindow.deal(1);
					//     // return false;
					// },
					btn1:function (index, layero) {
						var iframeWin = layero.find('iframe')[0];
						iframeWin.contentWindow.deals();
					},
					// btn2: function (index, layero) {
					// 	var iframeWin = layero.find('iframe')[0];
					// 	iframeWin.contentWindow.deal(2);
					// 	return false;
					// },
					// yes: callback,
			...
			},lostRecord: function deal(title, url, width, height, callback) {
				layer.open({
					type: 2,
					area: ['90%', '95%'],
					fix: false,
			...
			// 确认窗体
			preConfirm: function (content, callBack) {
        ...
        operate: {
            ...
            get: function(url, callback) {
				...
				var _width = $.common.isEmpty(width) ? '90%' : width;
				var _height = $.common.isEmpty(height) ? '95%' : height;
				//如果是移动端，就使用自适应大小弹窗
				if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
					_width = 'auto';
					_height = 'auto';
			...
			}, saveSuccessTwo: function (result) {
				if (result.code == web_status.SUCCESS) {
					parent.window.layer.closeAll();
					parent.window.$.table.refresh();
			...
			// 消息提示并刷新父窗体(头像)
			msgReloadAvatar: function(msg, type) {
				layer.msg(msg, {
						icon: $.modal.icon(type),
						time: 500,
						shade: [0.1, '#8F8F8F']
			...
			// 保存信息并刷新网页
			saveAndFreshAll: function(url, data) {
				$.modal.loading("正在处理中，请稍后...");
				var config = {
					url: url,
					type: "post",
					...
					data: data,
					success: function(result) {
						$.operate.saveSuccessAvatar(result);
					}
				};
				$.ajax(config)
			},
			...
			// 保存结果提示msg(头像)
			saveSuccessAvatar: function (result) {
				if (result.code == web_status.SUCCESS) {
					$.modal.msgReloadAvatar("保存成功,正在刷新数据请稍后……", modal_status.SUCCESS);
				} else {
					$.modal.alertError(result.msg);
				}
				$.modal.closeLoading();
